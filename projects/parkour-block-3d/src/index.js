    ...
    return function (Godot) {
      ...
      function writeSockaddr(sa, family, addr, port, addrlen) {
        switch (family) {
          case 2:
            ...
            (tempI64 = [
              0 >>> 0,
              ((tempDouble = 0), +Math.abs(tempDouble) >= 1 ? (tempDouble > 0 ? (Math.min(+Math.floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0 : ~~+Math.ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0) : 0),
            ]),
      ...
      var GodotAudio = {
        ...
        init: function (mix_rate, latency, onstatechange, onlatencyupdate) {
          const ctx = new (window.AudioContext || window.webkitAudioContext)({ sampleRate: mix_rate });
          GodotAudio.ctx = ctx;
