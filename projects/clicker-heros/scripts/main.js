...
("use strict");
{
  ...
  window.RuntimeInterface = class RuntimeInterface {
    ...
    async _Init(opts) {
      ...
      if (opts.workerScripts) for (const [url, blob] of Object.entries(opts.workerScripts)) this._workerScriptBlobURLs[url] = URL.createObjectURL(blob);
...
("use strict");
{
  ...
  const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
    ...
    _OnWindowResize() {
      ...
      this._PostRuntimeEvent("window-resize", { innerWidth: width, innerHeight: height, devicePixelRatio: window.devicePixelRatio });
    ...
    _OnSimulatedResize(originalWidth, originalHeight, count) {
      ...
      const height = this._GetWindowInnerHeight();
      this._simulatedResizeTimerId = -1;
      if (width != originalWidth || height != originalHeight) this._PostRuntimeEvent("window-resize", { innerWidth: width, innerHeight: height, devicePixelRatio: window.devicePixelRatio });
    ...
    _OnKeyEvent(name, e) {
      ...
      this._PostToRuntimeMaybeSync(name, { code: code, key: e.key, which: e.which, repeat: e.repeat, altKey: e.altKey, ctrlKey: e.ctrlKey, metaKey: e.metaKey, shiftKey: e.shiftKey, timeStamp: e.timeStamp }, DISPATCH_RUNTIME_AND_SCRIPT);
    }
    _OnMouseWheelEvent(name, e) {
      this.PostToRuntime(name, { clientX: e.clientX, clientY: e.clientY, pageX: e.pageX, pageY: e.pageY, deltaX: e.deltaX, deltaY: e.deltaY, deltaZ: e.deltaZ, deltaMode: e.deltaMode, timeStamp: e.timeStamp }, DISPATCH_RUNTIME_AND_SCRIPT);
    }
...
("use strict");
{
  if (window["C3_IsSupported"]) {
    ...
    window["c3_runtimeInterface"] = new self.RuntimeInterface({ useWorker: enableWorker, workerMainUrl: "workermain.js", engineScripts: ["scripts/c3runtime.js"], scriptFolder: "scripts/", workerDependencyScripts: [], exportType: "html5" });
  }
}
("use strict");
{
...
("use strict");
{
  self.C3WebAudioInstance = class C3WebAudioInstance extends self.C3AudioInstance {
    ...
    Resume() {
      ...
      this._UpdateVolume();
      this._UpdatePlaybackRate();
      this._startTime = this.GetCurrentTime() - this._resumePosition / (this._playbackRate || 0.001);
